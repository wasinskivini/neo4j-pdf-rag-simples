...
Projeto de estudo que transforma o conteúdo de arquivos PDF em embeddings e armazena esses vetores no Neo4j, permitindo realizar buscas por similaridade semântica.
...
[!IMPORTANT]Este projeto implementa uma busca semântica vetorial.Ele ainda não é um RAG completo, pois não utiliza uma LLM para gerar uma resposta final a partir dos trechos recuperados.
...
Cada chunk é convertido em um vetor numérico que representa características semânticas do texto.
...
O Neo4j compara o vetor da pergunta com os vetores armazenados e devolve os chunks semanticamente mais próximos.

Isso significa que a busca não depende apenas de palavras idênticas. Uma pergunta pode recuperar um trecho relevante mesmo utilizando termos diferentes dos presentes no documento.
...
A aplicação retorna os trechos encontrados, mas ainda não produz uma resposta final como faria uma LLM.
...
[!WARNING]Não publique o arquivo .env.

Os nomes das variáveis devem corresponder exatamente aos nomes utilizados no código.
...
Para e remove os containers, preservando os volumes

npm run infra:reset

Remove containers e volumes, apagando os dados do Neo4j
...
Pode retornar algum chunk mesmo quando a pergunta não tem relação com o PDF.
...
Os chunks podem não possuir uma ordem explícita no grafo.

A quantidade de documentos processados ainda pode ser limitada.

🛠️ Possíveis melhorias

Adicionar uma pontuação mínima de similaridade.
...
O valor ideal do score deve ser calibrado utilizando perguntas relacionadas e não relacionadas ao conteúdo.
...
[!NOTE]Adicionar uma pasta ao .gitignore não remove arquivos que já estavam sendo rastreados pelo Git.

Para remover arquivos do índice sem apagá-los do computador:

git rm -r --cached neo4j/data neo4j/logs
...
Fundamentos necessários para construir um RAG
...
Os resultados representam os trechos considerados mais próximos da pergunta pelo modelo de embeddings.

Isso não garante que todo resultado seja realmente relevante ou suficiente para responder à consulta.
